...
object Lab5 extends jsy.util.JsyApplication with Lab5Like {
  ...
  def mapFirstWith[W,A](l: List[A])(f: A => Option[DoWith[W,A]]): DoWith[W,List[A]] = l match {
    ...
    case h :: t => f(h) match {
      case None => mapFirstWith(t)(f) map { a => h :: a }
...
//    case (_, TInterface(tvar, t2p)) => ???
    ...
    e match {
      ...
      case Binary(Plus, e1, e2) =>
        ...
        (t1, t2) match {
          ...
          case (TNumber|TString, _) => err(t2, e2) // FIXME check
        }
      case Binary(Minus|Times|Div, e1, e2) =>
        ...
        (t1, t2) match {
          case (TNumber, TNumber) => TNumber
          case (_, TNumber) => err(t1, e1) // FIXME possible
          case (TNumber, _) => err(t2, e2)
        }
      case Binary(Eq|Ne, e1, e2) =>
        val t1 = typeof(env, e1)
        val t2 = typeof(env, e2)
        if (t1 == t2 && !hasFunctionTyp(t1)) TBool
        else err(t1, e1) // FIXME throw errors for t2 when appropriate
      case Binary(Lt|Le|Gt|Ge, e1, e2) =>
        ...
        (t1, t2) match {
          ...
          case (TNumber|TString, _) => err(t2, e2) // FIXME check that errors are thrown properly
        }
      ...
      case If(e1, e2, e3) =>
        ...
        (t1, t2, t3) match {
          ...
          case (_, _, _) => err(t2, e2) // FIXME possible should it error on t2 if t2 and t3 dont match?
        }
      case Obj(fields) =>
      ...
      case GetField(e1, f) =>
        ...
        t1 match {
          case TObj(fields) => fields.get(f) match {
            case Some(exp) => exp
            case None => err(t1, e1) // FIXME possible
          }
          case _ => err(t1, e1)
        }
      /***** Cases from Lab 4 that need a small amount of adapting. */
      case Decl(m, x, e1, e2) if isBindex(m, e1) =>
        val mt1 = MTyp(m, typeof(env, e1)) // FIXME check, do we want to wrap in an MTyp?
      ...
      case Call(e1, args) => typeof(env, e1) match {
        case TFunction(params, tret) if params.length == args.length =>
          (params zip args).foreach {
            // Call(e1: Expr, args: List[Expr])
            // TFunction(params: List[(String,MTyp)], tret: Typ)
  ...
  def getBinding(mode: Mode, e: Expr): DoWith[Mem,Expr] = {
    ...
    (mode, e) match {
      ...
      case (MRef, _) if !isLValue(e)   => doreturn(e)
    }
  }
  ...
  def step(e: Expr): DoWith[Mem, Expr] = {
    ...
    e match {
      ...
      case Binary(Seq, v1, e2) if isValue(v1) =>
        doreturn(e2)
      case Binary(Plus, S(str1), S(str2)) =>
        doreturn(S(str1 + str2))
      ...
      case Call(v @ Function(p, params, _, e1), args) =>
        ...
        if (pazip.forall { case ((_, MTyp(m, _)), ep) => !isRedex(m, ep) }) {
          val dwep = pazip.foldRight( doreturn(e1) : DoWith[Mem,Expr] )  {
            case (((stri, MTyp(mi, _)), argi), dwacc) => getBinding(mi,argi) flatMap {
              e1p => dwacc map {
                ebod => substitute(ebod, e1p, stri)
              }
            }
          }
          p match {
          ...
          val dwpazipp = mapFirstWith(pazip) {
            case (pi @ (_, MTyp(m, _)), argi) if isRedex(m, argi) => Some(step(argi) map { ap => (pi, ap) })
            case _ => None
          }
          dwpazipp map {
